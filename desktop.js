...
console.log('%cWindows 12 网页版(GitHub: clay-psy)', 'background-image: linear-gradient(to right, #ad6eca, #3b91d8); border-radius: 8px; font-size: 1.3em; padding: 10px 15px; color: #fff; ');
...
let nts = {
    ...
    'feedback': {
        cnt: `<p class="tit">反馈</p>
            <p>我们非常注重用户的体验与反馈</p>
            <list class="new">
                <a class="a" onclick="window.open('https://github.com/clay-psy');" win12_title="在浏览器新窗口打开链接" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)">在github上提交issue(需要github账户，会得到更高重视)</a>
                <a class="a" onclick="window.open('https://github.com/clay-psy');" win12_title="在浏览器新窗口打开链接" onmouseenter="showdescp(event)" onmouseleave="hidedescp(event)">在Microsoft Forms上发送反馈(不需要账户，也会重视)</a>
            </list>`,
        btn: [
            { type: 'main', text: '关闭', js: 'closenotice();' },
        ]
...
let apps = {
    setting: {
        ...
        theme_get: () => {
            ...
            // 实时获取主题
            $.get('https://github.com/clay-psy').then(cs => {
                cs.forEach(c => {
                    if (c.type == 'dir') {
                        $.get(c.url).then(cnt => {
                            $('#set-theme').html('');
                            cnt.forEach(cn => {
                                if (cn.name == 'theme.json') {
                                    $.getJSON('https://github.com/clay-psy' + cn.path).then(inf => {
                                        infjs = inf;
                                        if ($('#set-theme>loading').length)
                                            $('#set-theme').html('');
                                        $('#set-theme').append(`<a class="a act" onclick="apps.setting.theme_set('${c.name}')" style="background-image:url('https://github.com/clay-psy${c.name}/view.jpg')">${c.name}</a>`);
                                    });
                                }
                            });
                        });
        ...
        theme_set: (infp) => {
            $.get('https://github.com/clay-psy').then(cnt => {
                console.log('https://github.com/clay-psy');
                cnt.forEach(cn => {
                    if (cn.name == 'theme.json') {
                        $.getJSON('https://github.com/clay-psy').then(inf => {
                            infjs = inf;
                            cnt.forEach(fbg => {
                                console.log(fbg, infjs);
                                if (fbg.name == infjs.bg) {
                                    $(':root').css('--bgul', `url('https://github.com/clay-psy')`);
                                    $(':root').css('--theme-1', infjs.color1);
                                    $(':root').css('--theme-2', infjs.color2);
                                    $(':root').css('--href', infjs.href);
                                    // $('#set-theme').append(`<a class="a act" onclick="apps.setting.theme_set(\`(${inf})\`)" style="background-image:url('https://github.com/clay-psy')">${c.name}</a>`);
                                }
                            });
                        });
                    }
...
document.getElementsByTagName('body')[0].onload = () => {
    ...
    $.getJSON('https://github.com/clay-psy').then(j=>{
        if(j.sp){
            $(':root').css('--bgul',j.bg);
            if(j.spth){
